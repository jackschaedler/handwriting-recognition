...
function clearVis() {
  vis.selectAll(".smooth_points").data(grail.SmoothData()).remove();
...
var smooth_path = vis.append("path")
...
function update()
{
  var points = background.selectAll(".raw_points").data(grail.RawData());
  ...
  var smooth_points = vis.selectAll(".smooth_points").data(grail.SmoothData());

  smooth_points.enter().append("circle")
      .attr("class", "smooth_points")
      .attr("cx", function(d) { return xRange(d.x); })
      .attr("cy", function(d) { return yRange(d.y); })
      .attr("fill", "black")
      .attr("stroke", "black")
...
function poll()
{
  ...
  var NEW_SMOOTHING_AMT = SMOOTHING_TANGLE.getValue("smoothing_pct");

  if (NEW_SMOOTHING_AMT != grail.SmoothingAmount())
  {
    clearVis();
    grail.SetSmoothingAmount(NEW_SMOOTHING_AMT);
    update();
  }
}

var helpIcon = d3.select('#smooth_wrapper').append("span")
